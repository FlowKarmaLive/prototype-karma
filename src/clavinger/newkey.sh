...
openssl req -new -key ${TMPDIR}/${NAME}.key -out ${TMPDIR}/${NAME}.csr -subj "/C=US/ST=CA/L=San Francisco/O=FlowKarma.Live/OU=${FROM}/CN=${NAME}" && \
...
openssl pkcs12 -export -out ${KEYDIR}/${NAME}.pfx -inkey ${TMPDIR}/${NAME}.key -in ${TMPDIR}/${NAME}.pem -certfile ca.pem -passout pass:
